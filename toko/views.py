...
class SortListView(generic.ListView):
    template_name = 'sort.html'
    queryset = ProdukItem.objects.all().order_by('harga')
    paginate_by = 8

class DescListView(generic.ListView):
    template_name = 'sort.html'
    queryset = ProdukItem.objects.all().order_by('-harga')
    paginate_by = 8
...
class TopstListView(generic.ListView):
    template_name = 'tops.html'
    queryset = ProdukItem.objects.filter(kategori='T')
    paginate_by = 8

class BottomstListView(generic.ListView):
    template_name = 'bottoms.html'
    queryset = ProdukItem.objects.filter(kategori='B')
    paginate_by = False

class DressListView(generic.ListView):
    template_name = 'dresses.html'
    queryset = ProdukItem.objects.filter(kategori='D')
    paginate_by = False

class NewListView(generic.ListView):
    template_name = 'new.html'
    queryset = ProdukItem.objects.filter(label='NEW')
    paginate_by = False

class SaleListView(generic.ListView):
    template_name = 'sale.html'
    queryset = ProdukItem.objects.filter(label='SALE')
    paginate_by = False

class BestListView(generic.ListView):
    template_name = 'best.html'
    queryset = ProdukItem.objects.filter(label='BEST')
    paginate_by = False
